...
def train(model, train_loader, optimizer, device, num_epochs):
    for epoch in tqdm.tqdm(range(num_epochs)):
        for data in train_loader:
            ...
            x = encode_label_in_image(x.view(-1), y)
            loss = model.train(x, True, optimizer)

            x = encode_label_in_image(x.view(-1), create_negative_data())
...
def predict(model, x):
    ...
    for i in range(10):
        x = encode_label_in_image(x.view(-1), torch.tensor([i]))
        goodness, _ = model(x)
